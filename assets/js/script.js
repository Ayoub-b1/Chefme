...
async function is_admin() {
    return new Promise((resolve, reject) => {
        let xhttp = new XMLHttpRequest();
        xhttp.open('GET', '/api/user/is_admin/', true);
        xhttp.send();
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                let response = JSON.parse(this.responseText);
                if (response.status == 'success') {
                    ...
                    if (response.is_admin == 1) {
                        setCookie('is_admin', response.token, 1);
                        ...
                        newLink.setAttribute("aria-label", "New Link");
                        newLink.setAttribute("href", '/admin/?token=' + response.token);
                        newLink.textContent = "Admin Panel";

                        newListItem.appendChild(newLink);

                        var navbarNav = document.querySelector(".navbar-nav");
                        navbarNav.insertBefore(newListItem, document.getElementById("logout").parentNode);

                        resolve(); // Resolve the Promise once done
                    } else {
                        resolve(); // Resolve the Promise even if user is not admin
                    }
                } else {
