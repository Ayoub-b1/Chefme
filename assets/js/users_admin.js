...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function send_delete(id) {
        ...
        xhttp.open("POST", "/api/user/delete/", true);
    ...
    function displayrecipes(data, place, filters) {
                ...
                return filters.every(filter => {
                    ...
                    switch (filter.name) {
                        case 'search':
                            ...
                            recipe.lastname.toLowerCase().includes(filter.value.toLowerCase());
        ...
        setTimeout(() => {
          ...
          <div class="shadow bg-white hover-scale hover-bg-danger rounded-5 h-100 position-relative d-flex flex-column align-items-center mb-4 ">
                                    <img src="../${filteredData[i].profile_pic}" class="recipeimgtop" alt="...">
                                    <div class="mt-5 d-flex flex-column align-items-center pt-4 px-4 text-center">
                                      <h6 class="bg-danger hover-bg-white text-white px-2 py-1 rounded-5">${filteredData[i].name} ${filteredData[i].lastname}</h6>
                                      ...
                                      <h6 class="mb-0 fs-lg-7 fs-6">${filteredData[i].email}</h6></div>
                ...
                card.addEventListener('dragstart', (event) => {
                    ...
                    deletelement(card, filteredData[i].id);
    ...
    function getUsers() {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (response.status == 'success') {
                    data = response.users
                    displayrecipes(response.users , document.querySelector('.recipes'));
                }
    ...
    document.querySelector('#add_admin').addEventListener('submit', function (event) {
        event.preventDefault();
        let form = event.currentTarget;
        let email = form.email.value;
        let xhttp = new XMLHttpRequest();
        xhttp.open("POST", "/api/user/add_admin/", true);
        let formData = new FormData();
